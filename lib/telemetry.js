...
Telemetry.prototype.handleBlock = function(payload)
{
	switch (payload[0])
	{
	case this.blocks['battery']:
		this.handleBattery(payload);
		break;
...
Telemetry.prototype.subscribe = function(group, subfunc)
{
	...
	case 'battery':
		this.startBattery();
		this.addListener('battery', subfunc);
		break;	
	case 'motor':
		this.startMotor();
...
Telemetry.prototype.startBattery = function()
{

	var self = this;
	if (this.blocks['battery'])
		return;
...
Telemetry.prototype.handleBattery = function(payload)
{
	var update =
	{
		level: payload.readFloatLE(4)
	};
